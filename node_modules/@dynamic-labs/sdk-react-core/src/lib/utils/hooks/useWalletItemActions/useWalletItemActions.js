...
const useWalletItemActions = () => {
    const { setMultiWalletWidgetState, setShowAuthFlow, setSelectedWalletConnector, wallets, setQrcodeUri, setDesktopUri, user, linkedWallets, setLegacyIsVerifying, } = useInternalDynamicContext();
    ...
    const handleCustodialWalletClick = (walletConnector) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (twoStepAuthentication) {
            setSelectedWalletConnector(walletConnector);
    ...
    const openWallet = (walletKey, handlers) => {
        ...
        const wallet = wallets.find((wallet) => wallet.key === walletKey);
        if (!wallet)
            return;
        if (isMobile()) {
            openMobileWallet(wallet.walletConnector);
        }
        else if (wallet.walletConnector.canConnectViaCustodialService) {
            openCustodialWallet(wallet.walletConnector);
        }
        else if (wallet.isInstalledOnBrowser) {
            openExtensionWallet(wallet.walletConnector);
        }
    };
