...
const useHandleWalletsToConnect = () => {
    const { walletsToConnectByChain, setWalletsToConnectByChain, setShowAuthFlow, setIsFullyConnected, isFullyConnected, } = useInternalDynamicContext.useInternalDynamicContext();
    const { setView, goToInitialView } = ViewContext.useViewContext();
    const handleWalletsToConnect = ({ walletConnector, }) => {
        if (!walletsToConnectByChain) {
        ...
        if (walletsToConnectByChain.length === 0) {
            setShowAuthFlow(false);
        ...
        const restOfWallets = walletsToConnectByChain.filter((wallet) => wallet.chain !== (walletConnector === null || walletConnector === void 0 ? void 0 : walletConnector.connectedChain));
        setWalletsToConnectByChain(restOfWallets);
