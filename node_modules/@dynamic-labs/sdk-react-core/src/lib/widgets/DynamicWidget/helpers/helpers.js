...
const MIN_BALANCE_VALUE = 0.0001;
const WITHOUT_DECIMAL_VALUE = 1000;
const MAX_BALANCE_VALUE = 1000000;
const PRECISION_FLOOR_DIVIDER = 1 / MIN_BALANCE_VALUE;
const roundBalance = (balanceString) => {
    ...
    if (clearedBalance < MIN_BALANCE_VALUE && clearedBalance > 0)
        return `< ${MIN_BALANCE_VALUE}`;
    const roundedBalance = smartFloor(clearedBalance * PRECISION_FLOOR_DIVIDER) /
        PRECISION_FLOOR_DIVIDER;
    if (roundedBalance === 0)
        return '0.0';
