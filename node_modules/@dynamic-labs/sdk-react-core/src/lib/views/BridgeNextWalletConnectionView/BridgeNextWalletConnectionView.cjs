...
const BridgeNextWalletConnectionView = () => {
    const { setShowAuthFlow, walletsToConnectByChain } = useInternalDynamicContext.useInternalDynamicContext();
    const { setView } = ViewContext.useViewContext();
    if (!(walletsToConnectByChain === null || walletsToConnectByChain === void 0 ? void 0 : walletsToConnectByChain.length)) {
        throw new utils.DynamicError('No wallets to connect');
    }
    const [nextWalletToConnect] = walletsToConnectByChain;
