...
const BridgeWelcomeView = () => {
    const { appName, walletsByChain = [], setShowAuthFlow, } = useInternalDynamicContext.useInternalDynamicContext();
    const { setView } = ViewContext.useViewContext();
    const { t } = reactI18next.useTranslation();
    const body = React.useMemo(() => {
        const blockchainNames = walletsByChain.map(({ chain }) => { var _a; return (_a = walletConnectorCore.getChainInfo(chain)) === null || _a === void 0 ? void 0 : _a.blockchainName; });
    ...
    const chains = React.useMemo(() => walletsByChain
        .map(({ chain }, index) => {
        ...
        .filter(isChainInfoDefined), [walletsByChain]);
    return (jsxRuntime.jsx(BridgeWelcomeLayout.BridgeWelcomeLayout, { onClickClose: () => setShowAuthFlow(false), title: t('dyn_bridge.welcome_view.title', { appName }), body: body, chains: chains, actionButtonText: t('dyn_bridge.welcome_view.button'), onClickAction: () => setView('wallet-list') }));
};
