...
const SocialRedirectContextProvider = ({ children, }) => {
    var _a, _b, _c, _d;
    const { setShowAuthFlow, wallets, setSelectedWalletConnector, walletUiUtils, consumeNonce, displaySiweStatement, environmentId, projectSettings, setIsSingleWalletAccount, setShowDynamicUserProfile, isAuthenticated, setPrimaryWalletId, secondaryWallets, sdkHasLoaded, authToken, connectWallet, primaryWallet, socialProvidersFilter = (providers) => providers, } = useInternalDynamicContext.useInternalDynamicContext();
    ...
    const socialWalletConnector = wallets
    ...
    React.useEffect(() => {
        ...
        if (params.has('magic_oauth_request_id')) {
            ...
            setShowAuthFlow(true);
            if (!isAuthenticated) {
                setSelectedWalletConnector(socialWalletConnector);
            }
