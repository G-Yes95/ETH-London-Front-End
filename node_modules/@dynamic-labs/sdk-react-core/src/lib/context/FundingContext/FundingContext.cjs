...
const FundingContextProvider = ({ children, }) => {
    ...
    const value = React.useMemo(() => ({
        ...
        showFunding,
        supportsFunding,
    }), [fundingEnabled, fundingUrl, showFunding, supportsFunding]);
    React.useEffect(() => {
        const updateSupportsFunding = () => _tslib.__awaiter(void 0, void 0, void 0, function* () {
            var _a;
            if (!fundingEnabled)
                return;
            if (!primaryWallet)
                return;
            const chainName = (_a = walletConnectorCore.getChainInfo(primaryWallet.chain)) === null || _a === void 0 ? void 0 : _a.name;
            if (!chainName)
                return;
            try {
    ...
    }, [primaryWallet, network, environmentId, fundingEnabled]);
